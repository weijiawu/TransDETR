...
class DetMOTDetection:
    ...
    def _pre_single_frame(self, idx: int):
        ...
        if self.vis:
            ...
            for posint in labels:
                x_min,y_min,x_max,y_max = posint[2:6]
                save_img = cv2.rectangle(save_img, (int(x_min),int(y_min)), (int(x_max),int(y_max)), (0,0,255), thickness=5)
            cv2.imwrite("./exps/show/{}".format(img_path.split("/")[-1]),save_img)
#             assert False
...
def build_dataset2transform(args, image_set):
    ...
    dataset2transform_train = {'ICDAR2015': mot17_train, 'YVT': mot17_train, 'COCOTextV2': crowdhuman_train, "ICDAR15_Pre": crowdhuman_train}
    dataset2transform_val = {'ICDAR2015': mot17_test, 'YVT': mot17_test,'COCOTextV2': mot17_test,"ICDAR15_Pre": crowdhuman_train}
    if image_set == 'train':
