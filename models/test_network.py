...
from models.deformable_transformer_plus import build_deforamble_transformer
from models.qim import build as build_query_interaction_layer
from models.memory_bank import build_memory_bank
from models.deformable_detr import SetCriterion, MLP
...
class RuntimeTrackerBase(object):
    def __init__(self, score_thresh=0.5, filter_score_thresh=0.3, miss_tolerance=3):
...
#                 for a, b in zip(outputs_class[:-1], outputs_coord[:-1])]
    ...
    def forward(self, frame):
        ...
        if not isinstance(frame, NestedTensor):
            samples = nested_tensor_from_tensor_list([frame])
...
args.update_query_pos = True
args.merger_dropout = 0
args.dropout = 0
args.random_drop = 0.1
args.fp_ratio = 0.3
args.query_interaction_layer = 'QIM'
args.extra_track_attn = True



net,_,_ = build(args)
net = net.cuda()
inputs = torch.randn( 3, 800, 800).cuda()
