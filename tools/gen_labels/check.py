...
for seq in tqdm(seqs):
    ...
    bboxess, IDss = parse_xml(ann_path,osp.join(image_path_frame,"{}.jpg".format(1)))
    ...
    for i in range(len(IDss)):
        ...
        for ic, (bboxes,IDs) in enumerate(zip(bboxess[i],IDss[i])):
            track_id = int(IDs)  
            
            for boxweijia in bboxess[i][ic+1:]:
#                 print(calculate_iou_polygen(bboxes,boxweijia))
                if calculate_iou_polygen(bboxes,boxweijia)>0.8:
                    lines.append(frame_path_one+"\n")
                    print(frame_path_one)
#                     break

write_lines("./check.txt", lines)                      
