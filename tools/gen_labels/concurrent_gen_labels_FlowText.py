...
#         print(box)
    ...
    return bboxes_box, IDs, rotates, words_final,bboxes_bonding
...
#     root = tree.getroot()  # 获取树型结构的根  VideoSynthText  FlowText
    
    from_label_root = "/mmu-ocr/weijiawu/Data/VideoText/MOTR/FlowTextV2/images/train"
    seq_root = '/mmu-ocr/weijiawu/Data/VideoText/MOTR/FlowTextV2/images/train'
    label_root = '/mmu-ocr/weijiawu/Data/VideoText/MOTR/FlowTextV2/labels_with_ids/train'

    ann_path = os.path.join(from_label_root, seq, "ann.json")
    image_path_frame = osp.join(seq_root,seq)
    
    for ii in os.listdir(image_path_frame):
        if "jpg" in ii:
            name = ii
            break
    image_path = osp.join(image_path_frame,name)
...
#             executor.map(getBboxesAndLabels_icd13(seqs,seq_root),image_files)
    ...
    return bboxess, IDss, rotatess,wordss, orignial_bboxess, seq

def mkdirs(d):
    if not osp.exists(d):
        os.makedirs(d)

def gen_data_path(path,split_train_test="train",data_path_str = "./datasets/data_path/FlowTextV2.train"):
    ...
    for video_name in os.listdir(image_path):
        ...
        for i in range(0,len(frame_list)):
            frame_real_path = "FlowTextV2/images/train/" + video_name + "/{}.jpg".format(str(i).zfill(8)) + "\n"
            lines.append(frame_real_path)
    write_lines(data_path_str, lines)  
    
def gen_label(seq,seq_root):
    image_path_frame = osp.join(seq_root,seq)
    seq_label_root = osp.join(label_root, seq)
    mkdirs(seq_label_root)
    
    ann_path = os.path.join(from_label_root, seq, "ann.json")
    bboxess, IDss, rotatess, wordss,orignial_bboxess = parse_xml(ann_path,osp.join(image_path_frame,"00000000.jpg"))
    ID_list = {}
    
    for i in range(len(IDss)):
        frame_id = i
        label_fpath = osp.join(seq_label_root, '{}.txt'.format(str(frame_id).zfill(8)))
...
tid_curr =0 
with Pool(max_workers=20) as execute :
    seqs = [s for s in os.listdir(seq_root)]
#     seqs = seqs[:10000]

    future_task = [execute.submit(parse_xml,seq) for seq in seqs]
...
#         try:
        ...
        if ret:
            f_ret = f.result()
            bboxess, IDss, rotatess, wordss,orignial_bboxess,seq = f_ret
...
# gen_data_path(path="/mmu-ocr/pub/weijiawu/Data/VideoText/MOTR/VideoSynthText")