...
def gen_data_path(path,split_train_test="train",data_path_str = "./datasets/data_path/UnrealText.train"):
    ...
    for video_name in os.listdir(image_path):
        ...
        for frame_path_ in os.listdir(frame_path):
            ann = os.path.join("/mmu-ocr/weijiawu/Data/VideoText/MOTR/UnrealText/labels_with_ids/train",video_name,frame_path_.replace(".jpg",".txt"))
...
#             if idx>50000:
#                 break
            frame_real_path = "UnrealText/images/train/" + video_name + "/{}".format(i) + "\n"
            lines.append(frame_real_path)
    write_lines(data_path_str, lines)  
    

from_label_root = "/mmu-ocr/weijiawu/Data/UnrealText/"
seq_root = '/mmu-ocr/weijiawu/Data/VideoText/MOTR/UnrealText/images/train'
label_root = '/mmu-ocr/weijiawu/Data/VideoText/MOTR/UnrealText/labels_with_ids/train'
mkdirs(label_root)
seqs = [s for s in os.listdir(seq_root)]

gen_data_path(path = "/mmu-ocr/weijiawu/Data/VideoText/MOTR/UnrealText")
...
#     from_label_root_one = osp.join(from_label_root, seq,"labels")
#     mkdirs(seq_label_root)
#     ann_list = os.listdir(from_label_root_one)
    
#     for a in tqdm(ann_list):
#         ann_path = os.path.join(from_label_root_one,a)
#         boxs = []
#         words = []
...
#         boxs = load_dict["bbox"]
#         words = load_dict["text"]
        
# #         for line in lines:
# #             box = list(map(int, line.rstrip('\n').lstrip('\ufeff').split(',')[:8]))
# #             boxs.append(box)
# #             words.append(line.rstrip('\n').lstrip('\ufeff').split(',')[-1])
#         one_imgs_path = os.path.join(image_path_frame,a.replace("gt_","").replace("json","jpg"))
        
#         if not os.path.isfile(one_imgs_path):
#             continue
...
#         label_fpath = osp.join(seq_label_root, a.replace("gt_","").replace("json","txt"))
        
#         if os.path.isfile(label_fpath):
#             continue

                
#         lines = []
                    
#         for box,word in zip(boxs,words):
#             tid_curr += 1
